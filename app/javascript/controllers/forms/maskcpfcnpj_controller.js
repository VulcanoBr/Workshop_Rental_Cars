...
export default class extends Controller {
  ...
  addMaskToCpfCnpjFields() {
    ...
    if (value_cpfcnpj.length <= 11) {
      mask = value_cpfcnpj.replace(/(\d{3})(\d)/, "$1.$2").replace(/(\d{3})(\d)/, "$1.$2").replace(/(\d{3})(\d{1,2})$/, "$1-$2")
    } else {
      mask = value_cpfcnpj.replace(/(\d{2})(\d)/, "$1.$2").replace(/(\d{3})(\d)/, "$1.$2").replace(/(\d{3})(\d)/, "$1/$2").replace(/(\d{4})(\d)/, "$1-$2")
    }
