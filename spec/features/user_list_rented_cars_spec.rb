...
feature 'View' do
  describe 'list' do
    ...
    let!(:car) { create(:car, car_model_id: car_model.id, subsidiary_id: subsidiary.id, status: 'rented') }
    let!(:car2) { create(:car, car_model_id: car_model2.id, subsidiary_id: subsidiary.id, status: 'rented') }
    ...
    let!(:rental) { create(:scheduled_rental, car_id: car.id, user_id: user.id, 
              customer_id: customer.id, rented_code: '2024TYR56RTUYIU', daily_price: 234.56, status: :active) }
    let!(:rentals) { create(:scheduled_rental, car_id: car2.id, user_id: user.id, 
              customer_id: customer2.id, rented_code: '2024TYR56RTUBAD', daily_price: 334.56, status: :active) }

    it ' @rented_rental all' do
    ...
    it ' @rented_cars not found rented code' do
      ...
      allow(Rental).to receive(:rented).with("204RTREYY").and_return([])
      expect(current_path).to eq rented_cars_rentals_path
      expect(page).to have_content('Carro(s) Alocado(s)')
      expect(page).to have_button('Buscar')
      expect(page).to have_content('Código de Locação')
      expect(page).to have_content("Nenhuma locação encontrada para '204RTREYY' !!!.")
    end

    it ' @rented_cars found rented code' do
      ...
      allow(Car).to receive(:rented).with("Model").and_return(rentals)
      expect(current_path).to eq rented_cars_rentals_path
      expect(page).to have_content('Carro(s) Alocado(s)')
      expect(page).to have_button('Buscar')
      expect(page).to have_content('Código de Locação')
